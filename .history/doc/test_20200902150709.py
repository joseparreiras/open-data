...
possession_loc = pd.DataFrame([x for x in possession_data.location.values])

possession_loc.head()

type_list = pd.unique([x['name'] for x in possession_data.type])
...
for x in possession_data.iterrows():
    play = x[1]
