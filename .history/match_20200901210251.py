...
class match(object):
    ...
    def summary(self):
        ...
        for team in self.teams:
            ...
            shots_on_tgt = [
                x for x in shots if x['outcome'] in ['Goal', 'Saved']]
            shots_off_tgt = [x for x in shots if x['outcome'] in [
                'Off T', 'Post', 'Saved Off T', 'Saved To Post', 'Wayward']]
            shots_blocked = [x for x in shots if x['outcome'] == 'Blocked']
            goals = [x for x in shots if x['outcome']['name'] == 'Goal']

            shots_summary = {
                'shots': len(shots),
                'shots on target': len(shots_on_tgt),
                'shots off target': len(shots_off_tgt),
                'shots blocked': len(shots_blocked),
                'goals': len(goals)
            }
            summary_tbl[team].update(shots_summary)
            ...
            pass_summary = {
                'passes': len(passes),
                'passes completed': round(len(passes_comp)/len(passes)*100, 2),
                'passes incompleted': round(len(passes_incomp)/len(passes)*100, 2),
                'offside': len(passes_offside),
                'crosses': len(cross),
                'crosses completed': len(cross_comp),
                'crosses incompleted': len(cross_incomp),
            }

            summary_tbl[team].update(pass_summary)
        summry_tbl = pd.DataFrame(summary_tbl).T
        return summary_tbl
