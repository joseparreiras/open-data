    ...
    def summary(self):
        ...
        for team in self.teams:
            team_possession_idx = np.where(
                [x['name'] == team for x in self.data.possession_team])
            ...
            yellow_cards = [x for x in team_match.data.foul_committed.dropna(
            ) if 'card' in x.keys() and x['card']['name'] in ['Yellow Card', 'Second Yellow']]
            red_cards = [x for x in team_match.data.foul_committed.dropna(
